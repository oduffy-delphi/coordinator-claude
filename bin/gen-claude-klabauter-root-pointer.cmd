@echo off
setlocal
REM Windows launcher for gen-claude-klabauter-root-pointer.py — python-direct (NO bash re-exec).
...
REM   - On bare-name resolution, PowerShell prefers a "gen-claude-klabauter-root-pointer.ps1" sibling of
REM     this file over this "gen-claude-klabauter-root-pointer.cmd" file itself -- if both exist, `gen-claude-klabauter-root-pointer`
...
REM
REM Resolves a Python interpreter and runs the co-located entrypoint "gen-claude-klabauter-root-pointer.py"
...
echo [gen-claude-klabauter-root-pointer] ERROR: no Python interpreter found (python.exe / py -3). 1>&2
echo [gen-claude-klabauter-root-pointer] Install Python: https://www.python.org/downloads/windows/ 1>&2
exit /b 127
...
:run_baked
"%_py%" "%~dp0gen-claude-klabauter-root-pointer.py" %*
exit /b %ERRORLEVEL%

:run_py3
py -3 "%~dp0gen-claude-klabauter-root-pointer.py" %*
exit /b %ERRORLEVEL%
