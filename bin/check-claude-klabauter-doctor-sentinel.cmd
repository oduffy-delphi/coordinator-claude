@echo off
setlocal
REM Windows launcher for check-claude-klabauter-doctor-sentinel.sh — python-direct (NO bash re-exec).
...
REM   - On bare-name resolution, PowerShell prefers a "check-claude-klabauter-doctor-sentinel.ps1" sibling of
REM     this file over this "check-claude-klabauter-doctor-sentinel.cmd" file itself -- if both exist, `check-claude-klabauter-doctor-sentinel`
...
REM
REM Resolves a Python interpreter and runs the co-located entrypoint "check-claude-klabauter-doctor-sentinel.sh"
...
echo [check-claude-klabauter-doctor-sentinel] ERROR: no Python interpreter found (python.exe / py -3). 1>&2
echo [check-claude-klabauter-doctor-sentinel] Install Python: https://www.python.org/downloads/windows/ 1>&2
exit /b 127
...
:run_baked
"%_py%" "%~dp0check-claude-klabauter-doctor-sentinel.py" %*
exit /b %ERRORLEVEL%

:run_py3
py -3 "%~dp0check-claude-klabauter-doctor-sentinel.py" %*
exit /b %ERRORLEVEL%
