...
2. **Dispatch Haiku inventory agents (parallel)** — one per sub-chunk. Use the **Phase 1: Haiku Function-Level Inventory Prompt** from `plugins/coordinator/pipelines/deep-architecture-audit/agent-prompts.md`. These agents catalog what exists — no analysis. Pass scratch path `.claude/scratch/weekly-architecture-audit/{run-id}/{chunk-letter}{sub-chunk}-phase1-haiku.md` as `[SCRATCH_PATH]`. Include `Write` in each agent's tool list.
