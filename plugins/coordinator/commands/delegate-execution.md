...
1. Read post-mortem from stub's `## Execution Post-Mortem` section (written by the executor before exit)
2. Check the `Detection` field: `self` (executor caught it) produces more reliable diagnosis than `watchdog` (executor was unaware) — weight accordingly
3. Triage by the `Stub diagnosis` field:
