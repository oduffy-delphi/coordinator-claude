...
Generate `.claude/orientation_cache.md` — a compact summary for the SessionStart hook to inject in subsequent sessions instead of raw repomap/DIRECTORY content.

**Content derivation:**
1. **Structure:** Read `.claude/repomap.md`, extract top 15 entries by rank. Note total file count.
...
**If `.claude/` directory doesn't exist:** Skip. Not all repos use `.claude/`.
