...
- `[SCRATCH_PATH]` — `.claude/scratch/artifact-distillation/{run-id}/batch-{N}-phase1-haiku.md`

Include `Read`, `Write`, `Glob` in each agent's tool list. Dispatch with `run_in_background: true`.
...
- `[SCRATCH_PATH]` — `.claude/scratch/artifact-distillation/{run-id}/batch-{N}-phase1.5-qg.md`

Include `Read`, `Write`, `Glob` in each agent's tool list (Glob for path verification spot-checks). Dispatch with `run_in_background: true`.
...
- `[SCRATCH_PATH]` — `.claude/scratch/artifact-distillation/{run-id}/topic-{name}-phase2-sonnet.md`

Include `Read`, `Write` in each agent's tool list. Dispatch with `run_in_background: true`.
...
- `[SCRATCH_PATH]` — `.claude/scratch/artifact-distillation/{run-id}/phase3-opus-assembly.md`

Include `Read`, `Write`, `Glob` in the agent's tool list.
...
8. **Clean scratch:** `rm -rf .claude/scratch/artifact-distillation/{run-id}/`
