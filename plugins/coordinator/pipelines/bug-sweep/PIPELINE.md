...
6. **Generate run ID** — format: `YYYY-MM-DD-HHhMM` (current timestamp). This identifies the scratch directory: `.claude/scratch/bug-sweep/{run-id}/`
...
**Write-to-disk:** Each agent writes its complete findings to `.claude/scratch/bug-sweep/{run-id}/{chunk-name}-phase1-sonnet.md` using the Write tool. Include `Write` in the agent's tool list. The agent returns a brief summary (3-5 lines) to the coordinator: file written, finding count, any blockers. The coordinator reads full output from disk — agents do NOT return it in conversation.
...
**Write-to-disk:** The test agent writes its complete results to `.claude/scratch/bug-sweep/{run-id}/tests-phase1-haiku.md` using the Write tool. Include `Write` in the agent's tool list. Returns a brief summary (pass/fail counts, notable failures) to the coordinator. Full output read from disk.
...
**Scratch verification:** Before proceeding to Phase 2, verify all expected scratch files exist (`ls .claude/scratch/bug-sweep/{run-id}/`). If any chunk agent failed to write, re-dispatch once. If it fails again, proceed with available findings — the triage will cover fewer chunks but should not stall.

### Phase 2: Triage (Coordinator, ~5 min)

The coordinator reads all Phase 1 findings from `.claude/scratch/bug-sweep/{run-id}/` and categorizes.
