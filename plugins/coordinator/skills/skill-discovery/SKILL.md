...
**Commands** are invoked as `/command-name` via the Skill tool (e.g., `Skill tool → skill: "structured-research", args: "spec.yaml batch"`). They are dispatch workflows that run multi-phase agent pipelines.
...
| "The command exists, I'll follow it manually" | Invoke it. `Skill tool → skill: "structured-research", args: "spec.yaml batch"` — not Read + manual orchestration. Manual execution skips quality gates and scratch management. |
...
## Available Commands (invoke via Skill tool as `/command-name`)

Commands are dispatch workflows — multi-phase agent pipelines invoked with one command. **Use the Skill tool to invoke them, not the Read tool.**
