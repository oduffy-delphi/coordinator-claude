...
description: Use when work on a branch is ready to merge to main — creates PR, waits for CI, merges, cleans up. Invoke directly or via /workday-complete.
argument-hint: "[--force]"
---
...
- **If checks fail:** report which checks failed. Do NOT merge. Stop and report:
  _"CI failed on {check}. Fix the issue and re-run `/merge-to-main`, or investigate with `/systematic-debugging`."_
