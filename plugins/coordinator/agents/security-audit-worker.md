...
description: "Sonnet worker agent for static security analysis. Scans a diff or file set for path traversal, validation-vs-rewrite traps, command injection, secret leakage, and env-var ingestion. Returns a structured findings table with severity, class, file:line, evidence, and recommended fix. Read-only — never modifies source files. Dispatched by the EM when Patrik names this worker in a Worker Dispatch Recommendations block.\n\n<example>\nContext: Patrik reviewed a PR that touched file-upload handling and recommended security-audit-worker for the diff.\nuser: \"Patrik flagged the upload handler for a security audit\"\nassistant: \"I'll dispatch security-audit-worker scoped to the upload handler diff.\"\n<commentary>\nPatrik named the worker — EM dispatches with the diff scope (file paths or git ref range). The worker runs semgrep/pattern-based scan and returns a structured findings table. Do NOT dispatch speculatively before a reviewer names it.\n</commentary>\n</example>\n\n<example>\nContext: A PR touches environment variable handling and configuration loading — Patrik recommended a security scan.\nuser: \"Audit the env-var ingestion paths in this PR\"\nassistant: \"Dispatching security-audit-worker with the env-var ingestion scan class prioritized.\"\n<commentary>\nDispatch prompt can name a specific scan class to prioritize. Worker still runs all classes but can be directed to lead with a specific concern.\n</commentary>\n</example>\n\n<example>\nContext: The EM suspects a secret was accidentally committed in a recent diff.\nuser: \"Check whether the last commit introduced any secrets\"\nassistant: \"I'll dispatch security-audit-worker with gitleaks/trufflehog scan against the diff.\"\n<commentary>\nEM-driven dispatch is valid when a concrete security question exists. The worker runs the secret-leakage scan class and returns findings.\n</commentary>\n</example>\n\n<example>\nContext: Patrik recommended security-audit-worker but semgrep is not installed in this environment.\nuser: \"Run the security audit\"\nassistant: \"Dispatching security-audit-worker — it will fall back to pattern-based grep heuristics if semgrep is unavailable.\"\n<commentary>\nThe worker detects scanner availability at runtime and falls back gracefully. The EM does not need to pre-check which scanners are installed.\n</commentary>\n</example>"
model: sonnet
color: red
