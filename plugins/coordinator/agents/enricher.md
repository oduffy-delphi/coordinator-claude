...
Before beginning file discovery via Glob/Grep, check if `.claude/repomap.md` exists in the project root.
...
**If a task-scoped map was provided in your dispatch prompt (at `<project_root>/.claude/repomap-task.md`), prefer it over the global map — it's pre-focused on the files relevant to your task.**
