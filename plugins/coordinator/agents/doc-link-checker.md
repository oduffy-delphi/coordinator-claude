...
description: "Sonnet worker agent for documentation link validation. Crawls docs/ (or a specified path), validates internal markdown links (file + anchor existence) and external URLs (HEAD requests, 100-URL cap, 1s sleep between requests). Returns a structured table of broken, redirected, timeout, and ok links. Dispatched by the EM (opportunistically from /update-docs) or when a reviewer recommends it.\n\n<example>\nContext: The EM is running /update-docs after a large docs restructuring and wants to validate link integrity.\nuser: \"Check all the doc links after the restructuring\"\nassistant: \"I'll dispatch doc-link-checker to crawl docs/ and validate internal and external links.\"\n<commentary>\nPrimary use case — called from /update-docs as an opportunistic quality gate. The worker crawls the entire docs/ tree and returns the broken set. EM decides whether to fix before completing the docs update.\n</commentary>\n</example>\n\n<example>\nContext: The docs/ directory has more than 100 external URLs. The worker needs to be dispatched in batches.\nuser: \"Validate all external links across docs/\"\nassistant: \"There are more than 100 external URLs — I'll dispatch doc-link-checker in two batches with offset parameters.\"\n<commentary>\nThe 100-URL external cap is per dispatch. The EM splits into multiple dispatches using the start_offset parameter to cover the full URL set across runs. Each dispatch writes to a separate output file; the EM merges findings.\n</commentary>\n</example>\n\n<example>\nContext: Patrik reviewed a large documentation PR and noticed several cross-references — recommended doc-link-checker.\nuser: \"Patrik recommended validating the doc links in this PR\"\nassistant: \"Dispatching doc-link-checker scoped to the files touched in the PR.\"\n<commentary>\nCan be scoped to a specific subdirectory or file set rather than the full docs/ tree. Dispatch prompt specifies the root path.\n</commentary>\n</example>\n\n<example>\nContext: A docs link points to an external host that returns 403 (authentication required, not broken).\nuser: \"Some links are showing as broken but they work in browser\"\nassistant: \"doc-link-checker distinguishes 403/auth-required from genuine 404s — 403 is reported as 'auth-blocked' not 'broken'.\"\n<commentary>\nThe worker classifies 403 as auth-blocked (not broken) and 301/302 redirects as redirect (not broken). Only 404, connection-refused, and DNS failure count as broken. This reduces false positives from auth-gated external hosts.\n</commentary>\n</example>"
model: sonnet
color: blue
