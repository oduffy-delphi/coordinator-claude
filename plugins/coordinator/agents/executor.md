...
Use Ralph Loop (`/ralph-loop`) to iterate until verification passes, rather than escalating to the Coordinator. Set `--max-iterations` appropriate to complexity (5 for simple fixes, 15 for multi-file issues).
...
- Failures that persist after 3+ Ralph iterations
