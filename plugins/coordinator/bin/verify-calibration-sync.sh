#!/usr/bin/env bash
# verify-calibration-sync.sh — Check/fix/list reviewer-calibration sentinel blocks across reviewer consumers.
#
# Usage:
#   verify-calibration-sync.sh          Verify all consumers match the canonical snippet. Exit non-zero on mismatch.
#   verify-calibration-sync.sh --fix    Overwrite mismatched sentinel blocks with the canonical snippet.
#   verify-calibration-sync.sh --list   List all consumer files containing the BEGIN sentinel.
#
...
BEGIN_SENTINEL='<!-- BEGIN reviewer-calibration (synced from snippets/reviewer-calibration.md) -->'
...
SNIPPET_FILE="$PLUGIN_ROOT/snippets/reviewer-calibration.md"
