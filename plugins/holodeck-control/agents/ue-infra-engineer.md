...
| `manage_scalability` | device profiles, quality groups, platform defaults. Author-time config (vs manage_performance runtime CVars) |
| `manage_accessibility` | accessibility auditing (text, input, visual, audio), widget validation, batch defaults |
| `manage_modding` | mod descriptors, PAK files, content rules, compatibility auditing |
| `manage_build` | plugin management, build config, cook content, package project |
| `manage_subsystems` | subsystem inspection (Engine/Editor/World scopes), timer create/clear/list |
