...
2. **Parse plan status — frontmatter-first:**

   **Frontmatter parse rule (v2 plans):** If the file begins with `---` at byte 0 (line 1 is exactly `---`), read subsequent lines until the closing `---`. Parse `key: value` pairs only — flat, no nested YAML, no arrays, no multiline strings. The only recognized keys are `confidence` and `status`. Reject (with an error message) any value that looks like nested YAML, an array (`[...]`), or a multiline block. This is NOT a general YAML parser; it is a flat key-value frontmatter reader constrained to two fields.

   Valid `status` values: `draft` | `ready` | `executing` | `done`.
   Valid `confidence` values: `low` | `medium` | `high`.

   **Conflict resolution:** If both a frontmatter `status` field and a prose `**Status:**` line are present, frontmatter wins. Rewrite the prose `**Status:**` line to match (e.g., frontmatter `status: ready` → prose line becomes `**Status:** Ready for executor dispatch`).

   **Legacy fallback (v1 plans without frontmatter):** If no frontmatter block is present, scan for the prose `**Status:**` line and map it to a `status` enum value using this table:
   ...
   When updating a legacy plan, add a frontmatter block with the derived `status` value (and `confidence: low` as a conservative default) so future runs use the frontmatter path.

3. Check which items/phases are now implemented
4. Update status markers (e.g., checkbox completion, phase status, frontmatter `status` field)
5. If a plan is fully implemented, note the completion date and set frontmatter `status: done`
6. If implementation deviated from the plan, document what changed and why

**Test fixtures for this parse logic:** `coordinator/skills/writing-plans/examples/fixture-v1.md` (prose-only, legacy fallback path) and `coordinator/skills/writing-plans/examples/fixture-v2.md` (YAML frontmatter + intentionally stale prose line — frontmatter wins, prose is rewritten). Running this phase against both fixtures must produce correct status propagation with no silent drift.
