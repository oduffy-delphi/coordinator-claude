...
model: sonnet
color: blue
tools: ["Read", "Glob", "Grep", "Bash", "Edit", "Write", "ToolSearch", "WebFetch", "WebSearch", "mcp__plugin_context7_context7__resolve-library-id", "mcp__plugin_context7_context7__query-docs", "mcp__holodeck-docs__quick_ue_lookup", "mcp__holodeck-docs__lookup_ue_class", "mcp__holodeck-docs__check_ue_patterns", "mcp__holodeck-docs__find_symbol", "mcp__holodeck-docs__search_symbols", "mcp__holodeck-docs__search_ue_docs", "mcp__holodeck-docs__ue_mcp_status"]
...
- MCP tools — holodeck-docs for Unreal Engine API verification (class signatures, function declarations, anti-pattern checks): `quick_ue_lookup` for fast factual lookups, `lookup_ue_class` for exact class/method signatures, `check_ue_patterns` for anti-pattern detection, `find_symbol` for go-to-definition, `search_symbols` for workspace symbol search. **Lazy-loaded** — bootstrap before first use: `ToolSearch("select:mcp__holodeck-docs__quick_ue_lookup,mcp__holodeck-docs__lookup_ue_class,mcp__holodeck-docs__check_ue_patterns,mcp__holodeck-docs__find_symbol,mcp__holodeck-docs__search_symbols")`. Only available when holodeck-docs MCP server is running (UE projects). **Use these to verify UE API claims during enrichment** — never defer API verification to the executor when you can resolve it yourself.
