...
1. **Accumulated knowledge first.** Check what's already been mapped before searching from scratch:
   ...
   - **Subsystem profiles (UE projects):** `project_subsystem_profile()` — deterministic SQL profiles replacing Explore agent dispatches. List mode for discovery, profile mode for full context. <200ms per call vs. 150-300K tokens for an Explore agent.
